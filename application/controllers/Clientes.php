            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->db->table_exists('contas_receber')) {
                    if ($cliente_ativo == 0 && $this->core_model->get_by_id('contas_receber', array('conta_receber_cliente_id' => $cliente_id, 'conta_receber_status' => 0))) {
                        $this->session->set_flashdata('info', 'Esse cliente não pode ser desativado, pois está sendo utilizado em <i class="fas fa-hand-holding-usd"></i>&nbsp; Contas a Receber');
                        redirect('clientes');
                    }
                }
                ...
                //mostrar o nome do cliente que está sendo atualizado.
                $cliente = $this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id));

                $data = array(
                    'titulo' => 'Atualizar cadastro do Cliente ' . $cliente->cliente_nome,
                    'scripts' => array(
                        'vendor/mask/jquery.mask.min.js',
                        'vendor/mask/app.js',
                    ),
